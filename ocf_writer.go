...
func convertToArrayAndTruncate(ocfw *OCFWriter,data interface{}) ([]interface{}, error) {
	arrayValues, err := convertArray(data)
	if err != nil {
		return nil, err
	}
...
func GetAvroBytes(ocfw *OCFWriter, rawData interface{}) ([]byte, error) {
	var err error
	data, err := convertToArrayAndTruncate(ocfw, rawData)
	if err != nil {
		return nil, err
	}
	var block []byte // working buffer for encoding data values
	...
	for _, datum := range data {
		if block, err = ocfw.header.codec.BinaryFromNative(block, datum); err != nil {
			return nil, fmt.Errorf("cannot translate datum to binary: %v; %s", datum, err)
		}
	}
	...
	default:
		return nil, fmt.Errorf("should not get here: cannot compress block using unrecognized compression: %d", ocfw.header.compressionID)
