...
func Test_buildCodecForTypeDescribedByString_CacheRespectsPrecisionScale(t *testing.T) {
	...
	cacheHit, err := buildCodecForTypeDescribedByString(cache, "", "bytes", schemaMap, nil, o)
	...
	cacheMiss, err := buildCodecForTypeDescribedByString(cache, "", "bytes", schemaMap, nil, o)
	ensureError(t, err) // ensure NO error
