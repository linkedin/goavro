...
func intBinaryFromNativeOutput(out io.Writer, datum interface{}) error {
	var value int32
	switch v := datum.(type) {
	case int32:
		value = v
	case int:
		if value = int32(v); int(value) != v {
			return fmt.Errorf("cannot encode binary int: provided Go int would lose precision: %d", v)
		}
	case int64:
		if value = int32(v); int64(value) != v {
			return fmt.Errorf("cannot encode binary int: provided Go int64 would lose precision: %d", v)
		}
	case float64:
		if value = int32(v); float64(value) != v {
			return fmt.Errorf("cannot encode binary int: provided Go float64 would lose precision: %f", v)
		}
	case float32:
		if value = int32(v); float32(value) != v {
			return fmt.Errorf("cannot encode binary int: provided Go float32 would lose precision: %f", v)
		}
	default:
		return fmt.Errorf("cannot encode binary int: expected: Go numeric; received: %T", datum)
	}
	encoded := uint64((uint32(value) << 1) ^ uint32(value>>intDownShift))
	return integerBinaryEncoderOutput(out, encoded)
}
...
func longBinaryFromNativeOutput(out io.Writer, datum interface{}) error {
	var value int64
	switch v := datum.(type) {
	case int64:
		value = v
	...
	case float64:
		if value = int64(v); float64(value) != v {
			return fmt.Errorf("cannot encode binary long: provided Go float64 would lose precision: %f", v)
		}
	case float32:
		if value = int64(v); float32(value) != v {
			return fmt.Errorf("cannot encode binary long: provided Go float32 would lose precision: %f", v)
		}
	default:
		return fmt.Errorf("long: expected: Go numeric; received: %T", datum)
	}
	encoded := (uint64(value) << 1) ^ uint64(value>>longDownShift)
	return integerBinaryEncoderOutput(out, encoded)
}
...
func integerBinaryEncoderOutput(out io.Writer, encoded uint64) (err error) {
	// used by both intBinaryEncoder and longBinaryEncoder
	if encoded == 0 {
